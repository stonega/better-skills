...
export function printSkippedSkills(skipped: SkippedSkill[]): void {
  ...
  for (const [source, skills] of grouped) {
    ...
    console.log(`    ${DIM}To update: ${TEXT}npx @stonega/skills add ${source} -g -y${RESET}`);
  }
}
...
function printNewSkills(baseUrl: string, newSkills: string[], isGlobal: boolean): void {
  ...
  console.log(
    `    ${DIM}To install: ${TEXT}npx @stonega/skills add ${baseUrl} --skill ${names.join(' ')}${isGlobal ? ' -g' : ''}${RESET}`
  );
}
...
export async function updateGlobalSkills(
  ...
  if (skillNames.length === 0) {
    if (!options.skills) {
      ...
      console.log(
        `${DIM}Install skills with${RESET} ${TEXT}npx @stonega/skills add <package> -g${RESET}`
      );
    }
...
export async function updateProjectSkills(
  ...
  if (projectSkills.length === 0) {
    if (!options.skills) {
      ...
      console.log(
        `${DIM}Install project skills with${RESET} ${TEXT}npx @stonega/skills add <package>${RESET}`
      );
    }
...
export function printLegacyProjectSkills(
  ...
  for (const skill of legacy) {
    ...
    if (reinstall) {
      console.log(`    ${DIM}To refresh: ${TEXT}npx @stonega/skills add ${reinstall} -y${RESET}`);
    } else {
      console.log(
